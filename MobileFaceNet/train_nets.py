...
def get_parser():
    ...
    parser.add_argument('--image_size', nargs='+', default=[112, 112], help='the image size')
    ...
    parser.add_argument('--weight_decay', type=float, default=5e-5, help='L2 weight regularization.')
    parser.add_argument('--lr_schedule', nargs='+', help='Number of epochs for learning rate piecewise.', default=[4, 7, 9, 11])
    ...
    parser.add_argument('--show_info_interval', type=int, default=50, help='intervals to save ckpt file')
...
#         args.summary_path =  arg.output_path + 'summary' 
#         args.ckpt_path = arg.output_path + 'ckpt' 
#         args.log_file_path = arg.output_path + 'log'
        

        # create log dir
#         subdir = datetime.strftime(datetime.now(), '%Y%m%d-%H%M%S')
#         log_dir = os.path.join(os.path.expanduser(args.output_path + 'log'), subdir)
...
#             os.makedirs(log_dir)
        ...
        tfl = []
        for tfln in os.listdir(args.tfrecords_file_path):
            if tfln != '.ipynb_checkpoints':
                tfl.append(args.tfrecords_file_path + tfln)
        tfrecords_f = tfl
        
#         tfrecords_f = os.path.join(args.tfrecords_file_path, 'asia_112_train.tfrecord*')
...
#             ver_name_list.append(db)
        ...
        # summary writer
        summary = tf.summary.FileWriter(args.output_path + 'summary', sess.graph)
        summaries = []
        ...
        # output file path
#         if not os.path.exists(args.output_path + 'log'):
#             os.makedirs(args.output_path + 'log')
            ...
            while True:
                try:
                    ...
                    if count > 0 and count % args.ckpt_interval == 0:
                        ...
                        filename = os.path.join(args.output_path + 'ckpt', filename)
                        print('save ckpt files: ' + filename)
